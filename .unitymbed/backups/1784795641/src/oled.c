...
static void OLED_I2C_Init(void) {
    // Enable GPIOB clock
    RCC->AHBPERIPH_CLK_ENABLE |= RCC_AHBPERIPH_CLK_ENABLE_GPIOB; // ref: N32G031 RM p.102 (AHBPERIPH_CLK_ENABLE register)

    // Configure PB6 (SCL) and PB7 (SDA) as Open-Drain Output, High Speed
    // Clear PMODE bits for PB6 (bits 13:12) and PB7 (bits 15:14)
    GPIOB->PMODE &= ~(GPIO_PMODE_PMODE6_Msk | GPIO_PMODE_PMODE7_Msk); // ref: N32G031 RM p.138 (GPIOx_PMODE register)
    // Set PMODE bits for PB6 and PB7 to 01 (General purpose output mode)
    GPIOB->PMODE |= (0x1u << (6 * 2)) | (0x1u << (7 * 2)); 

    // Clear OTYPE bits for PB6 and PB7 to 1 (Open-drain output)
    GPIOB->OTYPE |= (GPIO_OTYPE_OTYPE6 | GPIO_OTYPE_OTYPE7); // ref: N32G031 RM p.139 (GPIOx_OTYPE register)

    // Clear OSPEED bits for PB6 and PB7 to 11 (High speed)
    GPIOB->OSPEED |= (GPIO_OSPEED_OSPEED6_Msk | GPIO_OSPEED_OSPEED7_Msk); // ref: N32G031 RM p.139 (GPIOx_OSPEED register)
