...
uint16_t adc_read_channel(uint8_t channel)
{
    // Set channel for first conversion in regular sequence (SQ1 in RSQ0)
    // SQ1 bits are 4:0 in ADC_RSQ0 (ref: N32G031 RM p.229, ADC_RSQ0 register)
    ADC->RSQ0 &= ~(0x1FU << 0); // Clear SQ1 (bits 4:0)
    ADC->RSQ0 |= (channel & 0x1F); // Set SQ1 to the desired channel
