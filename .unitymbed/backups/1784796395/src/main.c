...
int main(void)
{
    char buffer[20]; // Buffer for string formatting
    ...
    adc_init(); // Initialize ADC for PA0

    OLED_ShowString(10, 24, "System Ready", 16);
    delay_ms(1500); 
    
    OLED_Clear();
    OLED_ShowString(0, 0, "Volume:", 16); // Display "Volume:" label
    ...
    while (1)
    {
        ...
        if (volume_percent != last_volume_percent) {
            // Clear previous percentage value to prevent ghosting
            OLED_ShowString(0, 32, "           ", 16); // Clear area for percentage

            // Display current percentage value
            sprintf(buffer, "%3d%%", volume_percent); // Format as "  0%", " 50%", "100%"
            OLED_ShowString(0, 32, buffer, 16); // Show percentage at (0,32)

            last_volume_percent = volume_percent;
        }

        delay_ms(100); // Shorter delay for more responsive volume updates
    }
}
...
void adc_init(void)
{
    ...
    RCC->AHBPCLKEN |= (1U << 17); // Bit 17: IOPAEN (GPIOA clock enable) (ref: N32G031 RM p.102)

    // 2. Configure PA0 as analog input
    GPIOA->PMODE &= ~(0x3U << (0 * 2)); // Clear PMODE0 (bits 1:0)
    GPIOA->PMODE |= (0x3U << (0 * 2));  // Set PMODE0 to 11 (Analog mode) (ref: N32G031 RM p.138)

    // 3. Enable ADC clock
    RCC->APB2PCLKEN |= (1U << 9); // Bit 9: ADCEN (ADC clock enable) (ref: N32G031 RM p.104)

    // 4. Reset ADC
    RCC->APB2PRST |= (1U << 9);  // Bit 9: ADCRST (ref: N32G031 RM p.105)
    RCC->APB2PRST &= ~(1U << 9);
