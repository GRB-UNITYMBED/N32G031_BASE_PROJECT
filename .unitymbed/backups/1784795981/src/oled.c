...
static void OLED_I2C_Init(void) {
    // Enable GPIOB clock
    RCC->AHBPCLKEN |= (1U << 18); // Bit 18: IOPBEN (GPIOB clock enable) (ref: N32G031 RM p.102)
    ...
    // Set PMODE bits for PB6 and PB7 to 01 (General purpose output mode) (ref: N32G031 RM p.138)
    GPIOB->PMODE |= ((0x1U << (6 * 2)) | (0x1U << (7 * 2))); 

    // Set OTYPE bits for PB6 and PB7 to 1 (Open-drain output) (ref: N32G031 RM p.139)
    GPIOB->POTYPE |= ((1U << 6) | (1U << 7)); 

    // Set OSPEED bits for PB6 and PB7 to 11 (High speed) (ref: N32G031 RM p.139)
    GPIOB->OSPEEDR |= ((0x3U << (6 * 2)) | (0x3U << (7 * 2)));
